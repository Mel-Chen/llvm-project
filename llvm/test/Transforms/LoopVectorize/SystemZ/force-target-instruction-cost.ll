...
; CHECK:       [[VECTOR_BODY]]:
; CHECK-NEXT:    [[OFFSET_IDX:%.*]] = phi i64 [ 0, %[[VECTOR_PH]] ], [ [[IV_NEXT:%.*]], %[[VECTOR_BODY]] ]
; CHECK-NEXT:    [[TMP2:%.*]] = getelementptr inbounds i64, ptr [[DST]], i64 [[OFFSET_IDX]]
; CHECK-NEXT:    store i64 [[OFFSET_IDX]], ptr [[TMP2]], align 8
; CHECK-NEXT:    [[IV_NEXT]] = add nuw nsw i64 [[OFFSET_IDX]], 3
; CHECK-NEXT:    [[CMP:%.*]] = icmp ult i64 [[OFFSET_IDX]], 22
; CHECK-NEXT:    br i1 [[CMP]], label %[[VECTOR_BODY]], label %[[EXIT:.*]]
; CHECK:       [[EXIT]]:
; CHECK-NEXT:    ret void
;
entry:
