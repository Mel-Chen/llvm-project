...
; RUN: opt -S -passes=loop-vectorize -force-vector-width=4 -force-vector-interleave=1 -enable-interleaved-mem-accesses=true -enable-mem-access-versioning=true < %s | FileCheck %s --check-prefix=VF4UF1
; RUN: opt -S -passes=loop-vectorize -force-vector-width=1 -force-vector-interleave=2 -enable-interleaved-mem-accesses=true -enable-mem-access-versioning=true < %s | FileCheck %s --check-prefix=VF1UF2
; RUN: opt -S -passes=loop-vectorize -force-vector-width=2 -force-vector-interleave=2 -enable-interleaved-mem-accesses=true -enable-mem-access-versioning=true < %s | FileCheck %s --check-prefix=VF2UF2
