define i32 @zext_caniv_loop(ptr %base, ptr %out_addr, i32 %n) {
entry:
  %nz = icmp eq i32 %n, 0
...
vector.body:
  ...
  %vec.phi = phi <2 x i32> [ zeroinitializer, %entry ], [ %15, %vector.body ]
  ...
  %12 = shl i64 %11, 30
  %13 = getelementptr i8, ptr %base, i64 %12
  %14 = call <2 x i32> @llvm.experimental.vp.strided.load.v2i32.p0.i64(ptr align 4 %13, i64 1073741824, <2 x i1> splat (i1 true), i32 2)
  %15 = add <2 x i32> %vec.phi, %14
  %16 = ptrtoint <2 x ptr> %10 to <2 x i64>
  %17 = getelementptr i64, ptr %out_addr, i32 %index
  store <2 x i64> %16, ptr %17, align 8
...
exit.loopexit:
  %acc.next.lcssa = phi i32 [ %acc.next, %loop ], [ %19, %middle.block ]
  br label %exit
